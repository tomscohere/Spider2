...
def call_llm(payload):
    ...
    if model.startswith("gpt"):
        ...
        # Model should be in format cohere/command-a-03-2025
        model = model.split("/")[-1]
        print(model)
        ...
        client = CohereClient(os.environ["COHERE_API_KEY"])
        # client = CohereClient(os.environ["CO_API_KEY_STAGING"], base_url="https://stg.api.cohere.ai")
        ...
        for i, message in enumerate(messages):
            ...
            for part in message["content"]:
                cohere_message['content'] = part['text'] if part['type'] == "text" else ""
        ...
        for i in range(3):
            try:
                ...
                output_message = response.model_dump()['message']['content'][0]['text']
                return True, output_message
            except Exception as e:
                logger.error("Failed to call LLM: " + str(e))
                if hasattr(e, 'response') and e.response is not None:
